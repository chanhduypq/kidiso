...
class Spreadsheet_Excel_Reader {
	...
	function hyperlink($row,$col,$sheet=0) {
		// duc.vov fixbug   $link = $this->sheets[$sheet]['cellsInfo'][$row][$col]['hyperlink'];

		//var_dump($this->sheets[$sheet]['cellsInfo'][$row][$col]);
		$link = $this->sheets[$sheet]['cellsInfo'][$row][$col]['xfIndex'];
		if (!empty($link)) {
			return $link['link'];
		}
		return '';
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				for($i=0;$i<$rowspan;$i++) {
					for($j=0;$j<$colspan;$j++) {
						if ($i>0 || $j>0) {
							$this->sheets[$sheet]['cellsInfo'][$row+$i][$col+$j]['xfIndex']=1;
						}
					}
				}
				/// fixbug duc.vov 

				if(!empty($this->sheets[$sheet]['cellsInfo'][$row][$col])) {
					///var_dump($this->sheets[$sheet]['cellsInfo'][$row][$col]);
